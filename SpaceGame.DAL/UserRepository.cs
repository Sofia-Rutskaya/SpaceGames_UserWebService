...
namespace SpaceGame.DAL
{
    ...
    internal class UserRepository : IUserRepository
    {
        ...
        public async Task<User> GetByNickName(string nickName)
        {
            ...
            filter.AddCondition("nick", ScanOperator.Equal, nickName);
            var search = _userTable.Scan(filter);
            var user = await search.GetNextSetAsync();
            if(user.Count != 0)
            {
                return _context.FromDocument<User>(user.First());
            }
